...
export default function Home() {
  // Automated Articles read karne ka logic
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '40px 20px', display: 'flex', gap: '50px' }}>
        <div style={{ flex: 2 }}>
          ...
          {posts.length > 0 ? posts.map((post) => (
            <div key={post.slug} style={{ marginBottom: '40px', borderLeft: '4px solid #000', paddingLeft: '20px' }}>
              ...
              <h3 style={{ fontSize: '28px', margin: '0 0 10px 0', fontWeight: 'bold' }}>{post.title}</h3>
              <p style={{ color: '#555', lineHeight: '1.6' }}>Exploring behavioral science through AI-driven insights from habit research and global library data.</p>
              <a href={`/posts/${post.slug}`} style={{ color: '#2563eb', fontWeight: 'bold', textDecoration: 'underline', fontSize: '14px' }}>
                Read Full Analysis →
              </a>
            </div>
          )) : (
            <p style={{ color: '#888' }}>AI is analyzing new books... Check back in a few minutes.</p>
          )}
        </div>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ backgroundColor: '#2563eb', color: '#fff', padding: '30px', borderRadius: '15px', marginTop: '40px' }}>
            ...
            <p style={{ fontSize: '14px', opacity: 0.9 }}>Get 10-minute deep dives into influential books.</p>
